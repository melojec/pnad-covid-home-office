...
pnad_setembro <- read.csv("PNAD_COVID_092020.csv")

####Trabalhando com os Dados####
####Setembro####
#Criando os pesos e filtrando São luís

pnad_pesos09 <- pnad_setembro %>% as_survey_design(
  ids = UPA, 
  strata = Estrato, 
  weights = V1032, 
...
#Criando dataset para conferir pessoas em Home Office
# gráfico
grafhomsexcor09 <- ggplot(home_sexo_cor09, 
                          aes(
                            fill = cor, 
                            y = trab_home_office, 
                            x = sexo)) +
  ...
  labs(x = "Sexo", 
       fill = "Cor/Raça: ", 
       caption = "Fonte: Microdados da Pnad Covid19 - IBGE. Setembro 2020.",
...
# gráfico
grafeducor09 <- ggplot(home_edu_cor09[which(home_edu_cor09$home_office>0),], 
...
home_sexo_idade09 <- pnad_pesos09 %>%
  group_by(sexo, Idade) %>%
  summarise(
    home_office = survey_total(C013 == 1, na.rm = TRUE),
    mao_de_obra = survey_total(C001 == 1, na.rm = TRUE)
...
# gráfico
grafsexidade09 <- ggplot(home_sexo_idade09[which(home_sexo_idade09$home_office>0),], 
                         aes(fill = Idade, 
                             y = trab_home_office, 
                             x = sexo)) +
  geom_bar(position = "dodge", 
...
# Gráfico
graftipemp09 <- ggplot(home_emprego09[which(home_emprego09$home_office>0),], 
