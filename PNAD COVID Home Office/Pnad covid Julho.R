...
pnad_julho <- read.csv("PNAD_COVID_072020.csv")

####Trabalhando com os Dados####
####Julho####
#Criando os pesos e filtrando São luís

pnad_pesos07 <- pnad_julho %>% as_survey_design(
  ids = UPA, 
  strata = Estrato, 
  weights = V1032, 
...
#Criando dataset para conferir pessoas em Home Office
# gráfico
grafhomsexcor07 <- ggplot(home_sexo_cor07, 
                          aes(
                            fill = cor, 
                            y = trab_home_office, 
                            x = sexo)) +
  ...
  labs(x = "Sexo", 
       fill = "Cor/Raça: ", 
       caption = "Fonte: Microdados da Pnad Covid19 - IBGE. Julho 2020.",
...
# gráfico
grafeducor07 <- ggplot(home_edu_cor07[which(home_edu_cor07$home_office>0),], 
...
home_sexo_idade07 <- pnad_pesos07 %>%
  group_by(sexo, Idade) %>%
  summarise(
    home_office = survey_total(C013 == 1, na.rm = TRUE),
    mao_de_obra = survey_total(C001 == 1, na.rm = TRUE)
...
# gráfico
grafsexidade07 <- ggplot(home_sexo_idade07[which(home_sexo_idade07$home_office>0),], 
                         aes(fill = Idade, 
                             y = trab_home_office, 
                             x = sexo)) +
  geom_bar(position = "dodge", 
...
# Gráfico
graftipemp07 <- ggplot(home_emprego07[which(home_emprego07$home_office>0),], 
