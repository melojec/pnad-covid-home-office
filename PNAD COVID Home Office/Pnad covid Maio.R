...
# definir diretório
setwd("C:/Users/Melo/Desktop/RJoao/Repositório 1/PNAD COVID")
...
pnad_maio <- read.csv("PNAD_COVID_052020.csv")

####Trabalhando com os Dados####
####Maio####
#Criando os pesos e filtrando São luís

pnad_pesos05 <- pnad_maio %>% as_survey_design(
  ids = UPA, 
  strata = Estrato, 
  weights = V1032, 
...
#Criando dataset para conferir pessoas em Home Office
# gráfico
grafhomsexcor05 <- ggplot(home_sexo_cor05, 
                          aes(
                            fill = cor, 
                            y = trab_home_office, 
                            x = sexo)) +
  ...
  labs(x = "Sexo", 
       fill = "Cor/Raça: ", 
       caption = "Fonte: Microdados da Pnad Covid19 - IBGE. Maio 2020.",
...
# gráfico
grafeducor05 <- ggplot(home_edu_cor05[which(home_edu_cor05$home_office>0),], 
...
home_sexo_idade05 <- pnad_pesos05 %>%
  group_by(sexo, Idade) %>%
  summarise(
    home_office = survey_total(C013 == 1, na.rm = TRUE),
    mao_de_obra = survey_total(C001 == 1, na.rm = TRUE)
...
# gráfico
grafsexidade05 <- ggplot(home_sexo_idade05[which(home_sexo_idade05$home_office>0),], 
                         aes(fill = Idade, 
                             y = trab_home_office, 
                             x = sexo)) +
  geom_bar(position = "dodge", 
...
# Gráfico
graftipemp05 <- ggplot(home_emprego05[which(home_emprego05$home_office>0),], 
