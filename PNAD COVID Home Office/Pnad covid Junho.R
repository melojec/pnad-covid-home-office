...
pnad_junho <- read.csv("PNAD_COVID_062020.csv")

####Trabalhando com os Dados####
####Junho####
#Criando os pesos e filtrando São luís

pnad_pesos06 <- pnad_junho %>% as_survey_design(
  ids = UPA, 
  strata = Estrato, 
  weights = V1032, 
...
#Criando dataset para conferir pessoas em Home Office
# gráfico
grafhomsexcor06 <- ggplot(home_sexo_cor06, 
       aes(
         fill = cor, 
         y = trab_home_office, 
         x = sexo)) +
...
# gráfico
grafeducor06 <- ggplot(home_edu_cor06[which(home_edu_cor06$home_office>0),], 
...
home_sexo_idade06 <- pnad_pesos06 %>%
  group_by(sexo, Idade) %>%
  summarise(
    home_office = survey_total(C013 == 1, na.rm = TRUE),
    mao_de_obra = survey_total(C001 == 1, na.rm = TRUE)
...
# gráfico
grafsexidade06 <- ggplot(home_sexo_idade06[which(home_sexo_idade06$home_office>0),], 
       aes(fill = Idade, 
           y = trab_home_office, 
           x = sexo)) +
  geom_bar(position = "dodge", 
...
# Gráfico
graftipemp06 <- ggplot(home_emprego06[which(home_emprego06$home_office>0),], 
